...
export function ResultCard({ industry }: { industry: string }) {
  ...
  if (!mounted) {
    return (
      <div className="max-w-2xl mx-auto mt-8 rounded-2xl border border-border bg-card overflow-hidden text-left">
  ...
  return (
    ...
    >
          ...
          return (
            <div key={i} className="rounded-xl p-3.5 flex items-start gap-3 hover:bg-muted/60 transition-colors">
              ...
              <button
                ...
                className={`relative w-8 h-[18px] rounded-full transition-colors shrink-0 mt-0.5 cursor-pointer ${states[i] ? "bg-success" : "bg-border"}`}
              >
      ...
      <div className="px-6 py-4 border-t border-border flex items-center justify-between">
        ...
        <button className="h-9 px-4 rounded-md bg-foreground text-background text-[13px] font-medium flex items-center gap-1.5 hover:opacity-90 transition-opacity cursor-pointer">
