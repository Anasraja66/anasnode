...
import { Calendar as CalendarIcon, Clock, Phone, MapPin, User, ArrowUpRight, Search } from "lucide-react";
...
export default async function BookingsPage() {
  ...
  const upcomingBookings = bookings.filter(b => isFuture(b.endAt));
  const pastBookings = bookings.filter(b => isPast(b.endAt));
            ...
            {upcomingBookings.map((booking) => (
              <div key={booking.id} className="p-6 hover:bg-zinc-50/50 transition-colors flex flex-col md:flex-row gap-6 md:items-center">
                <div className="flex-shrink-0 flex flex-col items-center justify-center w-20 h-20 bg-blue-50/50 border border-blue-100 rounded-2xl text-[#0A6BFF]">
                  <span className="text-xs font-bold uppercase tracking-wider">{format(booking.startAt, "MMM")}</span>
                  <span className="text-2xl font-black leading-none mt-1">{format(booking.startAt, "d")}</span>
                </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-zinc-500 flex items-center gap-1.5 mt-1">
                        ...
                        {isToday(booking.startAt) && <span className="ml-2 text-xs font-semibold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full">Today</span>}
                        {isTomorrow(booking.startAt) && <span className="ml-2 text-xs font-semibold bg-blue-100 text-[#0A6BFF] px-2 py-0.5 rounded-full">Tomorrow</span>}
                      </p>
                    </div>
              ...
              <div key={booking.id} className="px-6 py-4 flex items-center justify-between hover:bg-zinc-50/50 transition-colors opacity-75">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-zinc-100 rounded-xl flex flex-col items-center justify-center text-zinc-500">
                    <span className="text-[10px] font-bold uppercase">{format(booking.startAt, "MMM")}</span>
                    <span className="text-lg font-black leading-none">{format(booking.startAt, "d")}</span>
                  </div>
                  <div>
                    <h3 className="text-sm font-semibold text-zinc-900">{booking.title}</h3>
                    <p className="text-xs text-zinc-500 mt-0.5">{booking.contactName} • {format(booking.startAt, "h:mm a")}</p>
                  </div>
                </div>
