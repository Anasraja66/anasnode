...
import { RefreshCw, Plug, MessageSquare, TrendingUp, Layers, PhoneCall } from "lucide-react";
...
export default function DashboardHome() {
  ...
  const itemVariants = { hidden: { y: 15, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8, ease: [0.16, 1, 0.3, 1] as const } } };

  return (
    <div className="px-4 py-6 md:px-10 md:pt-8 md:pb-8">
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="space-y-[28px] relative z-10">
        <OnboardingWizard />
        <div className="relative z-10 space-y-[28px] w-full font-sans">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between min-h-[59px] w-full mb-8 pt-4">
            ...
            <div className="mt-4 md:mt-0 shrink-0">
              <button onClick={() => window.location.reload()} className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#DDEBFF] text-[#0A6BFF] text-[13px] font-bold hover:bg-blue-100 transition-colors shadow-sm cursor-pointer">
          ...
          <motion.div variants={itemVariants} className="bg-white border border-zinc-200 rounded-xl p-6 shadow-sm relative z-20">
            ...
            <PromptBox
              ...
              onFixIssue={(id) => {
                ...
                if (diagnostics.length === 1) setPromptMode("edit");
              }}
              onGenerate={(data, prompt) => {
                if (data.success && data.workspace) {
                  const pending = { id: "wf_" + Math.random().toString(36).substring(7), name: data.workflowName || "AI Generated Workflow", workflowName: data.workflowName || "AI Generated Workflow", workflow: data.workspace, nodes: data.workspace.nodes || [], edges: data.workspace.edges || [], industry: data.industry || "general", prompt, features: data.features || [], createdAt: Date.now() };
                  localStorage.setItem("anaos_pending_workflow", JSON.stringify(pending));
                  window.dispatchEvent(new CustomEvent("anaos-open-onboarding", { detail: { prompt, pendingWorkflow: pending } }));
                } else alert("Failed to generate workflow. Please try again.");
              }}
            />
            <AnimatePresence>
              {showConnectors && (
                <motion.div initial={{ opacity: 0, y: 10, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="mt-4 w-full">
                  <div className="bg-[#FAFAFA] border border-zinc-200 rounded-xl p-3 flex flex-col sm:flex-row items-center gap-4 hover:border-zinc-300 transition-colors">
                    <div className="w-10 h-10 rounded-xl bg-white border border-zinc-200 flex items-center justify-center shrink-0 shadow-sm text-sky-500"><Plug className="w-5 h-5" /></div>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <button onClick={() => setShowConnectors(false)} className="text-[12px] font-medium text-zinc-500 hover:text-zinc-800 transition-colors font-sans">Dismiss</button>
                      <button onClick={() => window.dispatchEvent(new Event("anaos-open-onboarding"))} className="bg-[#0A6BFF] hover:bg-blue-600 text-white shadow-sm text-[12px] font-semibold px-4 py-1.5 rounded-lg transition-all shadow-sm font-sans">Connect</button>
                    </div>
                  </div>
                </motion.div>
              )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
            <motion.div variants={itemVariants} className="bg-[#DDEBFF] border-none rounded-xl p-6 space-y-6 shadow-sm transition-all duration-300 flex flex-col group">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-white text-[#0A6BFF] flex items-center justify-center shadow-sm"><MessageSquare className="w-5 h-5" /></div>
                ...
                {[
                  ...
                  return (
                    <Link href={c.href} key={c.name} className="bg-white border-none px-4 py-3.5 rounded-xl text-[13px] font-bold text-zinc-800 shadow-sm flex items-center justify-between hover:bg-zinc-50 transition-colors cursor-pointer group/item block">
                      <div className="flex items-center gap-3"><BrandIcon id={c.id} className="w-5 h-5 shrink-0" /><span className={isConnected ? "text-zinc-900" : "text-zinc-500"}>{c.name}</span></div>
                      {isConnected ? (
                        <div className="flex items-center gap-1.5"><span className="text-[10px] font-bold text-emerald-600 uppercase">Connected</span><div className="w-1.5 h-1.5 rounded-full bg-emerald-500" /></div>
            ...
            <div className="flex flex-col gap-6">
              <Link href="/dashboard/integrations" className="bg-[#DDEBFF] border-none rounded-xl p-6 shadow-sm flex flex-col items-center justify-center text-center relative overflow-hidden group transition-colors block">
                {(() => {
                  const isPhoneConnected = (recentWorkflow?.requiredIntegrations || []).some((i: string) => i.toLowerCase() === "phone" || i.toLowerCase() === "voice");
                  return (
                    <>
                      <div className="w-12 h-12 rounded-full bg-white text-[#0A6BFF] flex items-center justify-center mb-4 relative shadow-sm group-hover:scale-110 transition-transform"><PhoneCall className="w-5 h-5" /></div>
                      <div className="space-y-1">
                        <h3 className={`text-[15px] font-bold ${isPhoneConnected ? "text-zinc-900" : "text-zinc-500"}`}>Voice Agent</h3>
                        {isPhoneConnected ? (
                          <div className="flex items-center justify-center gap-1.5 mt-1"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /><p className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest">ACTIVE</p></div>
                        ) : <div className="flex items-center justify-center gap-1.5 mt-1"><p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest">DISCONNECTED</p></div>}
                      </div>
                    </>
                  );
                })()}
              ...
              <motion.div variants={itemVariants} className="bg-[#DDEBFF] border-none rounded-xl p-6 space-y-6 shadow-sm transition-all duration-300 flex-1 group">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-white text-[#0A6BFF] flex items-center justify-center shadow-sm"><TrendingUp className="w-5 h-5" /></div>
                  ...
                  {[
                    ...
                    return (
                      <Link href={c.href} key={c.name} className="bg-white border-none px-3 py-3 rounded-xl text-[12px] font-bold text-zinc-800 shadow-sm flex items-center gap-2.5 hover:bg-zinc-50 transition-colors cursor-pointer block">
                        <div className="flex items-center gap-2.5"><BrandIcon id={c.id} className="w-5 h-5 shrink-0" /><span className={`truncate ${isConnected ? "text-zinc-900" : "text-zinc-500"}`}>{c.name}</span></div>
                      </Link>
                    );
                  })}
                </div>
            ...
            <motion.div variants={itemVariants} className="bg-[#DDEBFF] border-none rounded-xl p-6 space-y-6 shadow-sm transition-all duration-300 flex flex-col group">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-white text-[#0A6BFF] flex items-center justify-center shadow-sm"><Layers className="w-5 h-5" /></div>
              ...
              <div className="space-y-3 flex-1">
                {[
                  ...
                  const isConnected = (recentWorkflow?.requiredIntegrations || []).map((i: string) => i.toLowerCase()).includes(c.id) || (c.id === "googlecalendar" && (recentWorkflow?.requiredIntegrations || []).map((i: string) => i.toLowerCase()).includes("calendar"));
                  return (
                    <Link href={c.href} key={c.name} className="bg-white border-none px-4 py-3.5 rounded-xl text-[13px] font-bold text-zinc-800 shadow-sm flex items-center justify-between hover:bg-zinc-50 transition-colors cursor-pointer group/item block">
                      <div className="flex items-center gap-3"><BrandIcon id={c.id} className="w-5 h-5 shrink-0" /><span className={isConnected ? "text-zinc-900" : "text-zinc-500"}>{c.name}</span></div>
                      {isConnected ? (
          ...
          <motion.div variants={itemVariants} className="pt-10 flex w-full overflow-x-auto scrollbar-none justify-start md:justify-center">
            <div className="inline-flex items-center gap-8 md:gap-14 border-b border-zinc-150 pb-px px-4 md:px-10 min-w-max mx-auto">
              {["OVERVIEW", "WHATSAPP", "INSTAGRAM", "VOICE"].map((tab) => (
                <button key={tab} className={`pb-4 text-[12px] font-semibold transition-all relative tracking-[0.2em] ${tab === "OVERVIEW" ? "text-[#0A6BFF] after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[2px] after:bg-[#0A6BFF]" : "text-zinc-400 hover:text-zinc-650"}`}>{tab}</button>
              ))}
            </div>
          </motion.div>
        </div>
