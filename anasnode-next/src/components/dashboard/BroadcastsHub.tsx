...
export function BroadcastsHub({ workspaceName }: { workspaceName: string }) {
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto px-6 py-10 relative z-10 pb-10">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-zinc-950 tracking-tight">Mass Broadcasts</h1>
      ...
      <div className="rounded-[32px] border border-zinc-200/50 bg-white/50 backdrop-blur-md p-8 space-y-6 shadow-sm hover:shadow-md transition-shadow">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-[16px] font-black text-zinc-950 tracking-tight uppercase tracking-widest">AI Campaign Generator</h2>
        </div>
        ...
        <div className="relative group">
          <textarea
            ...
            className="w-full text-[15px] font-medium border border-zinc-200/50 rounded-2xl px-5 py-4 focus:outline-none focus:border-zinc-950 focus:ring-4 focus:ring-zinc-100 transition-all resize-none bg-white shadow-inner"
          />
        </div>
      ...
      <details className="group rounded-2xl border border-zinc-200/50 bg-white/50 backdrop-blur-md overflow-hidden transition-all shadow-sm">
        <summary className="px-6 py-5 font-black text-zinc-900 cursor-pointer hover:bg-zinc-50/50 flex items-center justify-between list-none">
        ...
        <div className="px-6 pb-6 pt-2">
          <ul className="space-y-3">
            {META_BROADCAST_RULES.map((r) => (
              <li key={r} className="flex items-start gap-4 text-[13.5px] text-zinc-500 font-bold">
      ...
      <div className="rounded-[32px] border border-zinc-200/50 bg-white/50 backdrop-blur-md overflow-hidden shadow-sm">
        <div className="px-8 py-6 border-b border-zinc-100 bg-zinc-50/30 flex items-center justify-between">
          <h2 className="text-[15px] font-black text-zinc-900 uppercase tracking-[0.2em]">Active Drafts & History</h2>
          <span className="text-[11px] font-black px-4 py-1.5 rounded-full bg-white border border-zinc-200 text-zinc-500">
            ...
            <thead>
              <tr className="border-b border-zinc-100">
                {["Campaign Name", "Status", "Delivery", "Last Updated", ""].map((h) => (
                  <th key={h} className="px-8 py-5 text-[11px] font-black text-zinc-400 uppercase tracking-widest">
                    {h}
                  </th>
                ))}
              </tr>
                  ...
                  <tr key={c.id} className="hover:bg-white/60 transition-colors group">
                    <td className="px-8 py-6">
                      <Link
                        ...
                        className="text-[15px] font-black text-zinc-900 hover:text-blue-600 transition-colors tracking-tight"
                      >
                        {c.name}
                      </Link>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-2">
                        <span className="text-[15px] font-black text-zinc-900 tabular-nums">{c.sentCount}</span>
                        <span className="text-[11px] text-zinc-400 font-black uppercase">/ {c.dailyCap} SENT</span>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-[14px] text-zinc-400 font-bold">
                      {new Date(c.updatedAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                    </td>
                    <td className="px-8 py-6 text-right">
                      <button className="p-2.5 hover:bg-white rounded-xl text-zinc-300 hover:text-zinc-900 transition-all opacity-0 group-hover:opacity-100 cursor-pointer shadow-sm">
